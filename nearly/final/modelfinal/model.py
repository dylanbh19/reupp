...
class DataLoader:
    ...
    def find_call_volume_column(self, df: pd.DataFrame, date_col: str) -> str:
        """Find the call volume column with smart detection"""
        ...
        # Look for obvious call volume column names
        call_candidates = []
        for col in numeric_cols:
            col_lower = str(col).lower()
            if any(keyword in col_lower for keyword in ['call', 'volume', 'count', 'total']):
                call_candidates.append(col)
        
        # If found call-related columns, use the first one
        if call_candidates:
            call_col = call_candidates[0]
            LOG.info(f"Using call volume column: {call_col} (keyword match)")
            return call_col
        
        # Otherwise, find the column with reasonable call volume range
        best_col = None
        best_score = -1
        ...
        for col in numeric_cols:
            try:
                ...
                # Check if values are in reasonable call volume range (10-10000 typically)
                mean_val = values.mean()
                std_val = values.std()
                min_val = values.min()
                max_val = values.max()
                
                # Score based on reasonableness for call volumes
                score = 0
                
                # Reasonable mean (100-5000)
                if 50 <= mean_val <= 10000:
                    score += 3
                elif 10 <= mean_val <= 20000:
                    score += 2
                elif mean_val > 0:
                    score += 1
                ...
                LOG.warning(f"Error evaluating column {col}: {e}")
                continue
        
        if best_col is None:
            # Fallback: use first numeric column
            best_col = numeric_cols[0]
            LOG.warning(f"Using fallback call column: {best_col}")
        else:
            LOG.info(f"Using call volume column: {best_col} (best match score: {best_score})")
        
        return best_col
    
    def load_call_data(self) -> pd.DataFrame:
        """Load and process call volume data with robust detection"""
        ...
        try:
            ...
            call_col = self.find_call_volume_column(df, date_col)
            
            # Clean call volume data
            df[call_col] = pd.to_numeric(df[call_col], errors='coerce')
            df = df.dropna(subset=[call_col])
            
            # Remove obviously wrong values (negative or extremely large)
            df = df[df[call_col] >= 0]
            
            # Remove extreme outliers (> 10 standard deviations)
            if len(df) > 10:
                mean_calls = df[call_col].mean()
                std_calls = df[call_col].std()
                if std_calls > 0:
                    outlier_threshold = mean_calls + 10 * std_calls
                    df = df[df[call_col] <= outlier_threshold]
            
            # Create daily aggregated data
            df = df.set_index(date_col).sort_index()
            daily_calls = df.groupby(df.index.date)[call_col].sum()
            ...
            if daily_calls.max() < 1:
                raise ValueError("Call volumes appear to be zero or negative")
    ...
    def find_mail_columns(self, df: pd.DataFrame, date_col: str) -> Tuple[Optional[str], Optional[str]]:
        ...
        for col in remaining_cols:
            col_lower = str(col).lower()
            if any(keyword in col_lower for keyword in ['type', 'category', 'mail', 'class', 'description']):
                # Check if it's actually categorical (not numeric)
        ...
        for col in numeric_cols:
            col_lower = str(col).lower()
            if any(keyword in col_lower for keyword in ['volume', 'count', 'amount', 'qty', 'quantity', 'total']):
                volume_col = col
                break
        
        # If no obvious volume column, use first numeric column
        if volume_col is None and numeric_cols:
