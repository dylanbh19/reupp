#!/usr/bin/env python
“””
COMPREHENSIVE MAIL-TO-CALLS PREDICTION SYSTEM - MULTI-SCENARIO TESTING

Testing different data cleaning and feature approaches:

1. BASELINE: Current approach (as-is)
1. OUTLIER_REMOVED: Remove call volume outliers
1. DATE_FILTERED: Remove early 2024 with low mail data
1. ENHANCED_FEATURES: Different feature engineering
1. COMBINED: All improvements together
...
# ============ NEW: OUTLIER SETTINGS ============
"outlier_method": "iqr",  # 'iqr' or 'threshold'
"outlier_threshold": 2000,  # minimum acceptable call volume
"iqr_multiplier": 1.5,  # for IQR method

# ============ NEW: DATE FILTERING ============
"remove_before_date": "2024-03-01",  # Remove data before this date
"min_mail_threshold": 0.1,  # Remove dates where total mail < 10% of average
```
...
# NEW: OUTLIER REMOVAL FUNCTIONS

# ============================================================================

def remove_call_outliers(df, method=‘iqr’, threshold=2000, iqr_multiplier=1.5):
“”“Remove outliers from call data using specified method”””
safe_print(f”\n   Removing call outliers using {method} method…”)

```
original_len = len(df)

if method == 'threshold':
    # Remove calls below threshold
    df_clean = df[df['call_volume'] >= threshold].copy()
    removed = original_len - len(df_clean)
    safe_print(f"   Removed {removed} rows with calls < {threshold}")
    ...
    df_clean = df[(df['call_volume'] >= lower_bound) & (df['call_volume'] <= upper_bound)].copy()
    removed = original_len - len(df_clean)
    
    safe_print(f"   IQR bounds: [{lower_bound:.0f}, {upper_bound:.0f}]")
    safe_print(f"   Removed {removed} outliers")

else:
    df_clean = df.copy()
    safe_print(f"   Unknown method {method}, no outliers removed")

safe_print(f"   Data after outlier removal: {len(df_clean)} rows")
return df_clean
```

def filter_by_mail_volume(merged_data, min_threshold_pct=0.1):
“”“Remove dates where mail volume is suspiciously low”””
safe_print(f”\n   Filtering by mail volume (min {min_threshold_pct*100}% of average)…”)

```
mail_columns = [col for col in merged_data.columns if col not in ['date', 'call_volume']]
merged_data['total_mail'] = merged_data[mail_columns].sum(axis=1)

avg_mail = merged_data['total_mail'].mean()
min_mail = avg_mail * min_threshold_pct

df_clean = merged_data[merged_data['total_mail'] >= min_mail].copy()
removed = len(merged_data) - len(df_clean)

safe_print(f"   Average mail volume: {avg_mail:.0f}")
safe_print(f"   Minimum threshold: {min_mail:.0f}")
safe_print(f"   Removed {removed} low-mail days")

# Drop the temporary column
df_clean = df_clean.drop('total_mail', axis=1)

return df_clean
```

def filter_by_date(df, start_date=‘2024-03-01’):
“”“Remove data before specified date”””
safe_print(f”\n   Filtering data from {start_date} onwards…”)

```
original_len = len(df)
start_dt = pd.to_datetime(start_date)
df_clean = df[df['date'] >= start_dt].copy()
removed = original_len - len(df_clean)

safe_print(f"   Removed {removed} rows before {start_date}")
safe_print(f"   Data after date filter: {len(df_clean)} rows")
...
# DATA MANAGER WITH SCENARIO SUPPORT

# ============================================================================

class DataManager:
def **init**(self, scenario=‘baseline’):
self.call_data = None
self.mail_data = None
self.merged_data = None
self.scenario = scenario
self.output_dir = Path(CONFIG[“output_dir”]) / f”scenario_{scenario}”
...
def load_call_data(self):
    ...
    safe_print("=" * 80)
    safe_print(f"STEP 1A: LOADING CLEAN CALL DATA - SCENARIO: {self.scenario.upper()}")
    safe_print("=" * 80)
    ...
    df_clean = df_clean[df_clean['call_volume'] >= 0]  # Remove negative values
    ...
    if self.scenario in ['outlier_removed', 'combined']:
        df_clean = remove_call_outliers(df_clean, 
                                       method=CONFIG['outlier_method'],
                                       threshold=CONFIG['outlier_threshold'],
                                       iqr_multiplier=CONFIG['iqr_multiplier'])
    
    if self.scenario in ['date_filtered', 'combined']:
        df_clean = filter_by_date(df_clean, CONFIG['remove_before_date'])
...
def load_mail_data(self):
    ...
    if self.scenario in ['date_filtered', 'combined']:
        daily_mail = filter_by_date(daily_mail, CONFIG['remove_before_date'])
...
def merge_data(self):
    ...
    # SCENARIO-SPECIFIC FILTERING
    if self.scenario in ['outlier_removed', 'combined']:
        merged = filter_by_mail_volume(merged, CONFIG['min_mail_threshold'])
...
# KEEP ORIGINAL EDA AND MODEL CLASSES (with minor modifications)
...
def **init**(self, merged_data, top_mail_types, best_lag, scenario=‘baseline’):
self.data = merged_data
...
def create_features(self):
    ...
    safe_print("\n" + "=" * 80)
    safe_print(f"STEP 3: FEATURE ENGINEERING - SCENARIO: {self.scenario.upper()}")
    safe_print("=" * 80)
    
    safe_print(f"   Using lag: {self.best_lag} days")
    safe_print(f"   Top mail types: {len(self.top_mail_types)}")
    
    # Use enhanced features for certain scenarios
    if self.scenario in ['enhanced_features', 'combined']:
        return self.create_enhanced_features()
    else:
        return self.create_baseline_features()

def create_baseline_features(self):
    """Original feature engineering (as-is)"""
    features_list = []
    targets_list = []
    dates_list = []
...
def create_enhanced_features(self):
    ...
    safe_print("   Using ENHANCED feature engineering...")
    ...
    # Create features for each day
    max_lookback = max(14, self.best_lag)  # Longer lookback for enhanced features
    ...
    for i in range(max_lookback, len(self.data) - self.best_lag):
        ...
        for mail_type in self.top_mail_types[:5]:  # Focus on top 5
            if mail_type in self.data.columns:
                clean_name = mail_type.replace(' ', '').replace('-', '').replace('_', '')[:15]
                ...
                # Extended lag features (1-7 days)
                ...
                for window in [3, 5, 7, 14]:
                    if i >= window - 1:
                        window_data = self.data.iloc[i - window + 1:i + 1][mail_type]
                        feature_row[f"{clean_name}_avg{window}"] = window_data.mean()
                        feature_row[f"{clean_name}_std{window}"] = window_data.std()
                
                # Week-over-week change
                if i >= 7:
                    week_ago = self.data.iloc[i - 7][mail_type]
                    if week_ago > 0:
                        feature_row[f"{clean_name}_wow_change"] = (self.data.iloc[i][mail_type] - week_ago) / week_ago
                    else:
                        feature_row[f"{clean_name}_wow_change"] = 0
        
        # === ENHANCED TOTAL MAIL FEATURES ===
        total_mail = sum(self.data.iloc[i][mt] for mt in self.top_mail_types if mt in self.data.columns)
        feature_row['total_mail_today'] = total_mail
        
        # Extended total mail lags
        for lag in range(1, 8):
            if i >= lag:
                total_mail_lag = sum(self.data.iloc[i - lag][mt] for mt in self.top_mail_types if mt in self.data.columns)
                feature_row[f'total_mail_lag{lag}'] = total_mail_lag
        ...
        for window in [3, 5, 7, 14]:
            if i >= window - 1:
                ...
                for j in range(i - window + 1, i + 1):
                    ...
                    window_totals.append(window_total)
                feature_row[f'total_mail_avg{window}'] = np.mean(window_totals)
                feature_row[f'total_mail_std{window}'] = np.std(window_totals)
                feature_row[f'total_mail_max{window}'] = np.max(window_totals)
                feature_row[f'total_mail_min{window}'] = np.min(window_totals)
        ...
        for window in [3, 5, 7, 14]:
            if i >= window - 1:
                window_calls = self.data.iloc[i - window + 1:i + 1]['call_volume']
                feature_row[f'calls_avg{window}'] = window_calls.mean()
                feature_row[f'calls_std{window}'] = window_calls.std()
                feature_row[f'calls_trend{window}'] = (window_calls.iloc[-1] - window_calls.iloc[0]) / window
        
        # === ENHANCED TEMPORAL FEATURES ===
        feature_row['weekday'] = current_date.weekday()
        feature_row['month'] = current_date.month
        feature_row['day_of_month'] = current_date.day
        feature_row['week_of_month'] = (current_date.day - 1) // 7 + 1
        feature_row['is_monday'] = 1 if current_date.weekday() == 0 else 0
        feature_row['is_friday'] = 1 if current_date.weekday() == 4 else 0
        feature_row['is_month_start'] = 1 if current_date.day <= 7 else 0
        feature_row['is_month_end'] = 1 if current_date.day >= 25 else 0
        
        # Sine/cosine encoding for cyclical features
        feature_row['weekday_sin'] = np.sin(2 * np.pi * current_date.weekday() / 7)
        feature_row['weekday_cos'] = np.cos(2 * np.pi * current_date.weekday() / 7)
        feature_row['month_sin'] = np.sin(2 * np.pi * current_date.month / 12)
        feature_row['month_cos'] = np.cos(2 * np.pi * current_date.month / 12)
    ...
    safe_print(f"✅ Created {len(X.columns)} ENHANCED features from {len(X)} samples")
    safe_print(f"   Mail features: {len([c for c in X.columns if any(mt.replace(' ', '')[:10] in c for mt in self.top_mail_types)])}")
    safe_print(f"   Call history: {len([c for c in X.columns if 'calls' in c])}")
    safe_print(f"   Temporal: {len([c for c in X.columns if any(t in c for t in ['weekday', 'month', 'day', 'sin', 'cos'])])}")
...
# KEEP ORIGINAL MODEL BUILDER AND PREDICTION SYSTEM CLASSES
...
“”“Run a complete pipeline for a specific scenario”””
safe_print(f”\n\n{’=’*80}”)
safe_print(f”RUNNING SCENARIO: {scenario_name.upper()}”)
safe_print(f”{’=’*80}”)

```
try:
    # Data loading with scenario-specific processing
    data_manager = DataManager(scenario=scenario_name)
    ...
    if len(merged_data) < 30:
        safe_print(f"⚠️  WARNING: Only {len(merged_data)} days of data for {scenario_name}")
        return {'scenario': scenario_name, 'success': False, 'error': 'Insufficient data'}
    ...
    if len(X) < 30:
        safe_print(f"⚠️  WARNING: Only {len(X)} samples for modeling in {scenario_name}")
        return {'scenario': scenario_name, 'success': False, 'error': 'Insufficient samples'}
    ...
    if not best_model:
        return {
            'scenario': scenario_name,
            'success': False,
            'error': 'No acceptable model found'
        }
    ...
    return {
        ...
        'overfitting': results[best_name]['overfitting'],
    ...
    safe_print(f"❌ ERROR in scenario {scenario_name}: {str(e)}")
    import traceback
    traceback.print_exc()
...
safe_print(“COMPREHENSIVE MAIL-TO-CALLS PREDICTION SYSTEM - MULTI-SCENARIO TESTING”)
safe_print(”=” * 80)
safe_print(“TESTING APPROACHES:”)
safe_print(“1. BASELINE: Current approach (as-is)”)
safe_print(“2. OUTLIER_REMOVED: Remove call volume outliers”)
safe_print(“3. DATE_FILTERED: Remove early 2024 with low mail data”)
safe_print(“4. ENHANCED_FEATURES: Different feature engineering”)
safe_print(“5. COMBINED: All improvements together”)
safe_print(”=” * 80)
...
safe_print("\n\n" + "=" * 80)
safe_print("SCENARIO COMPARISON RESULTS")
safe_print("=" * 80)
...
for result in all_results:
    if result['success']:
        comparison_data.append({
            ...
            'Test MAPE': f"{result['test_mape']:.1f}%",
...
if comparison_data:
    ...
    safe_print(df_comparison.to_string(index=False))

# Find best scenario
successful_results = [r for r in all_results if r['success']]
if successful_results:
    best_scenario = max(successful_results, key=lambda x: x['test_r2'])
    
    safe_print(f"\n🏆 BEST SCENARIO: {best_scenario['scenario'].upper()}")
    safe_print(f"   Test R²: {best_scenario['test_r2']:.3f}")
    safe_print(f"   Test MAE: {best_scenario['test_mae']:.0f}")
    safe_print(f"   Test MAPE: {best_scenario['test_mape']:.1f}%")
    safe_print(f"   Data Points: {best_scenario['data_points']}")
    safe_print(f"   Features: {best_scenario['features']}")
    
    # Create comparison visualization
    create_scenario_comparison_plot(all_results)
    
    safe_print(f"\n✅ ALL RESULTS SAVED TO: {CONFIG['output_dir']}/")
    safe_print("   Each scenario has its own subdirectory with:")
    safe_print("   - EDA plots")
    safe_print("   - Model validation plots")
    safe_print("   - Trained model files")
...
def create_scenario_comparison_plot(results):
“”“Create a visual comparison of all scenarios”””
...
for result in results:
    if result['success']:
        ...
        mae_scores.append(result['test_mae'])
        mape_scores.append(result['test_mape'])
...
fig.suptitle('Scenario Comparison Results', fontsize=16, fontweight='bold')

# R² Comparison
bars1 = axes[0, 0].bar(scenarios, r2_scores, alpha=0.7, color='blue')
axes[0, 0].set_ylabel('Test R²')
axes[0, 0].set_title('Model Performance (R²)')
axes[0, 0].grid(True, alpha=0.3)
axes[0, 0].set_ylim(0, max(r2_scores) * 1.2 if r2_scores else 1)
...
# MAE Comparison
bars2 = axes[0, 1].bar(scenarios, mae_scores, alpha=0.7, color='green')
axes[0, 1].set_ylabel('Mean Absolute Error')
axes[0, 1].set_title('Prediction Error (MAE)')
axes[0, 1].grid(True, alpha=0.3)

# MAPE Comparison
bars3 = axes[1, 0].bar(scenarios, mape_scores, alpha=0.7, color='orange')
axes[1, 0].set_ylabel('MAPE (%)')
axes[1, 0].set_title('Percentage Error (MAPE)')
axes[1, 0].grid(True, alpha=0.3)

# Data Points
bars4 = axes[1, 1].bar(scenarios, data_points, alpha=0.7, color='purple')
...
safe_print(f"\n📊 Scenario comparison plot saved: {output_dir}/scenario_comparison.png")
```

if **name** == “**main**”:
result = main()
if result[‘success’]:
safe_print(f”\n🚀 BEST APPROACH IDENTIFIED: {result[‘best_scenario’][‘scenario’].upper()}”)
safe_print(”   Review the comparison results to choose the best approach for production!”)
else:
safe_print(”\n💥 ALL APPROACHES FAILED - Need to investigate data quality issues”)